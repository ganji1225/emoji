#!/usr/bin/env python3
"""全38種絵文字テスト - VoiceDesignモデル（caption付き）"""
import os
import sys
import json
import time
...
OUTPUT_DIR = Path(__file__).parent / "outputs" / "emoji_test_voicedesign"
...
results = []
total = len(EMOJIS)
print(f"=== VoiceDesign Full Test ({total} patterns) ===")
...
for i, (name, emoji, desc) in enumerate(EMOJIS):
    ...
    try:
        ...
        results.append({
            ...
            "caption": CAPTION, "success": False,
            "elapsed_sec": round(elapsed, 1), "file_size_kb": 0,
            "wav_path": out_wav, "error": "TIMEOUT",
        })
        ...
        results.append({
            ...
            "caption": CAPTION, "success": False,
            "elapsed_sec": round(elapsed, 1), "file_size_kb": 0,
            "wav_path": out_wav, "error": str(e),
        })
        print(f"  -> [ERROR] {e}")

# Save JSON report
report_path = OUTPUT_DIR / "test_report_voicedesign.json"
