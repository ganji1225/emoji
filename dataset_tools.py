...
def _write_csv(records: list[dict], path: Path) -> None:
    ...
    with open(path, "w", encoding="utf-8", newline="") as f:
        has_caption = any("caption" in rec and str(rec.get("caption", "")).strip() != "" for rec in records)
        has_speaker = any(
...
def _call_llm_voice_caption(
    ...
    try:
        ...
        caption = resp.choices[0].message.content.strip()
        caption = _strip_emoji_characters(caption)
        log_fn(f"  [VOICE CAP RAW] {repr(caption[:80])}")
        return caption
    except Exception as e:
...
def emoji_caption(
    ...
    *,
    ...
    if voice_design and "caption" not in fieldnames:
